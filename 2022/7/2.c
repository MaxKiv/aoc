...
#define MINIMUM_DIR_SIZE 30000000
...
void find_smallest_big_dir_size(fs_tree_t *tree, size_t *minimum_dir_size,
                                size_t *answer) {
  if (tree->filetype == fs_dir) {
    if (tree->size >= *minimum_dir_size) {
      if (tree->size < *answer) {
        *answer = tree->size;
      }
    }
  ...
  for (size_t i = 0; i < tree->num_children; i++) {
    find_smallest_big_dir_size(tree->children[i], minimum_dir_size, answer);
  }
}
...
int main() {
  ...
  // calculate answer
  fs_tree_t *root = fs_tree_find_root(tree);
  size_t del_dir_size = MINIMUM_DIR_SIZE - (TOTAL_FS_SIZE - root->size);
  find_smallest_big_dir_size(root, &del_dir_size, &answer);
